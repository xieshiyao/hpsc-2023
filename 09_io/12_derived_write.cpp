...
int main(int argc, char** argv)
{
	...
	MPI_Type_create_subarray(2, N, Nlocal, offset, MPI_ORDER_C, MPI_INT,
							 &MPI_SUBARRAY);
	MPI_Type_commit(&MPI_SUBARRAY);
	MPI_File_open(MPI_COMM_WORLD, "data.dat", MPI_MODE_CREATE | MPI_MODE_WRONLY,
				  MPI_INFO_NULL, &file);
	MPI_File_set_view(file, 0, MPI_INT, MPI_SUBARRAY, "native", MPI_INFO_NULL);
	auto tic = chrono::steady_clock::now();
	MPI_File_write_all(file, &buffer[0], Nlocal[0] * Nlocal[1], MPI_INT,
					   MPI_STATUS_IGNORE);
